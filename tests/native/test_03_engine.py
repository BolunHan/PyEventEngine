"""Contract tests for the pure-Python engine layer (event_engine.native.engine).

Mirrors the capi engine contract (EventEngine / EventEngineEx) against the
native implementation, guaranteeing behavioral parity between the layers.
"""
...
from event_engine.native import Empty, EventEngine, EventEngineEx, Full, PyTopic
from event_engine.native import engine as native_engine
...
class TestEngineRegistry(unittest.TestCase):
    ...
    def test_00_initial_state(self) -> None:
        ...
        try:
            ...
            self.assertEqual(engine.exact_topic_hook_map, {})
            self.assertEqual(engine.generic_topic_hook_map, {})
        finally:
            engine.clear()

    def test_01_default_capacity_matches_native_constant(self) -> None:
        """The default capacity agrees with the native module constant."""
        engine = EventEngine()
        try:
            self.assertEqual(engine.capacity, native_engine.DEFAULT_MQ_CAPACITY)
        finally:
            engine.clear()
    ...
    def test_02_register_and_unregister_hook(self) -> None:
        ...
        engine = EventEngine()
        from event_engine.native import EventHook

        topic = PyTopic("registry.hook")
        hook = EventHook(topic)
        engine.register_hook(hook)
        try:
            self.assertEqual(len(engine), 1)
            self.assertEqual(list(engine.exact_topic_hook_map.keys()), [topic.value])
    ...
    def test_04_unregister_missing_hook_raises_keyerror(self) -> None:
        ...
        try:
            with self.assertRaises(KeyError):
                engine.unregister_hook(PyTopic("registry.missing"))
        finally:
            engine.clear()
    ...
    def test_06_unregister_handler_removes_empty_hook(self) -> None:
        ...
        engine = EventEngine()
        topic = PyTopic("registry.cleanup")
    ...
    def test_07_unregister_handler_missing_logs_error(self) -> None:
        ...
        engine = EventEngine()
        with self.assertLogs(native_engine.LOGGER, level="ERROR") as cm:
            engine.unregister_handler(PyTopic("registry.none"), lambda a: None)
    ...
    def test_08_generic_hook_registry(self) -> None:
        ...
        engine = EventEngine()
        topic = PyTopic("registry.+generic")
        ...
        try:
            self.assertEqual(len(engine), 1)
            self.assertEqual(engine.exact_topic_hook_map, {})
            self.assertEqual(list(engine.generic_topic_hook_map.keys()), [topic.value])
        finally:
            engine.clear()
    ...
    def test_09_iterators(self) -> None:
        ...
        engine = EventEngine()
        exact_topic = PyTopic("registry.iter.exact")

        def handler(a):
            pass

        engine.register_handler(exact_topic, handler)
        engine.register_handler(PyTopic("registry.{name}"), handler)
        try:
            ...
            matched = engine[PyTopic("registry.generic")]
            self.assertEqual(len(matched), 1)
...
class TestEngineQueue(unittest.TestCase):
    ...
    def test_02_get_blocking_timeout_raises_empty(self) -> None:
        ...
        engine = EventEngine(capacity=8)
        engine.activate()  # native engine _get_message gates on active
        try:
    ...
    def test_04_put_nonexact_topic_raises_valueerror(self) -> None:
        ...
        try:
            with self.assertRaises(ValueError):
                engine.put(PyTopic("queue.+any"), 1)
            with self.assertRaises(ValueError):
                engine.publish(PyTopic("queue.+any"), (1,), {})
        finally:
            engine.clear()

    def test_05_publish_signature(self) -> None:
        """publish(topic, args, kwargs, max_spin=...) mirrors put."""
        engine = EventEngine(capacity=8)
        topic = PyTopic("queue.publish")
    ...
    def test_08_occupied_matches_queue(self) -> None:
        """occupied tracks enqueued messages exactly."""
        engine = EventEngine(capacity=4)
        topic = PyTopic("queue.state")
...
class TestEngineLoop(unittest.TestCase):
    ...
    def test_01_multiple_handlers_same_topic(self) -> None:
        ...
        engine = EventEngine(capacity=16)
        topic = PyTopic("loop.multi.handler")
        results = []
    ...
    def test_02_multiple_topics(self) -> None:
        ...
        engine = EventEngine(capacity=16)
        topic1 = PyTopic("loop.t1")
        topic2 = PyTopic("loop.t2")
        results = {"t1": [], "t2": []}
        done = threading.Event()
    ...
    def test_03_generic_topic_routing(self) -> None:
        ...
        engine = EventEngine(capacity=16)
        generic = PyTopic("order.{name}.status")
        published = PyTopic("order.new.status")
        received = []
        done = threading.Event()
    ...
    def test_04_exact_and_generic_both_fire(self) -> None:
        ...
        engine = EventEngine(capacity=16)
        exact = PyTopic("route.data")
        generic = PyTopic("route.{name}")
        received = []
        done = threading.Event()
    ...
    def test_05_handler_exception_does_not_crash_engine(self) -> None:
        ...
        try:
            ...
            with self.assertLogs(engine.logger, level="ERROR") as cm:
                engine.put(topic, 42)
                self._wait_for(done)
            self.assertIn("ValueError: engine test", "\n".join(cm.output))
            engine.stop()
    ...
    def test_07_clear_resets_hook_maps(self) -> None:
        ...
        engine.register_handler(PyTopic("loop.clear.exact"), handler)
        engine.register_handler(PyTopic("loop.+generic"), handler)
        engine.clear()
        self.assertEqual(len(engine), 0)
        self.assertEqual(engine.exact_topic_hook_map, {})
        self.assertEqual(engine.generic_topic_hook_map, {})
    ...
    def test_08_start_twice_warns(self) -> None:
        """start() on an active engine logs a warning."""
        engine = EventEngine(capacity=16)
        try:
            engine.start()
